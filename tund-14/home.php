<?php
require("../../../config_vp2019.php");
require("functions_user.php");
require("functions_main.php");

// loadProfile($_SESSION["userId"]);

// Sessioon
require("classes/Session.class.php");

SessionManager::sessionStart("vp", 0, "/~stenrau/", "greeny.cs.tlu.ee");

// Kui ei ole sisse loginud
if (!isset($_SESSION["userFirstname"])) {
    header("Location: page.php");
}

if (isset($_GET["logout"])) {
    session_destroy();
    header("Location: page.php");
    exit();
}

$userName = $_SESSION["userFirstname"] . " " . $_SESSION["userLastname"];
$database = "if19_sten_ra_1";

// Küpsised
// setcookie() peab olema enne <html> tag-i
setcookie("vpname", $_SESSION["userFirstname"] . " " . $_SESSION["userLastname"], time()+60*60*24*31, "/~stenrau/", "greeny.cs.tlu.ee", isset($_SERVER["HTTPS"]), true);

if (isset($_COOKIE["vpname"])) {
    echo $_COOKIE["vpname"];
} else {
    echo "Küpsiseid ei leitud";
}

// Lisame lehe päise
require("header.php");
?>

<body>
    <?php
    echo "<h1>" . $userName . "i veebileht!</h1>";
    ?>
    <p>
        See leht on loodud koolis õppetöö raames ja ei sisalda tõsiseltvõetavat sisu!
    </p>
    <hr>
    <p><a href="?logout=1">Logi välja</a></p>
    <br>
    <p><a href="userprofile.php">Kasutaja profiil</a></p>
    <br>
    <p><a href="messages.php">Sõnumid</a></p>
    <br>
    <p><a href="picupload.php">Pildi üleslaadimine</a></p>
    <br>
    <p><a href="publicgallery.php">Avalike piltide galerii</a></p>

</body>

</html>